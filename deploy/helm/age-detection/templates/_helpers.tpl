...
{{- define "fd.env" -}}
- {name: AD_ENVIRONMENT, value: prod}
- {name: AD_LOG_JSON, value: "true"}
- {name: AD_MODEL_PATH, value: /app/models/face_detection_yunet_2026may.onnx}
- {name: AD_AGE_MODEL_PATH, value: /app/models/age_googlenet.onnx}
...
- {name: AD_AUTH_DISABLED, value: "true"}
{{- else }}
- name: AD_API_KEY_HASHES
  valueFrom:
    secretKeyRef:
...
- {name: AD_ASYNC_ENABLED, value: "true"}
- {name: AD_NATS_URL, value: {{ include "fd.natsUrl" . | quote }}}
- {name: AD_NATS_REPLICAS, value: {{ .Values.async.replicas | quote }}}
- {name: AD_NATS_STREAM, value: {{ .Values.async.stream | quote }}}
- {name: AD_NATS_CONSUMER, value: {{ .Values.async.consumer | quote }}}
- {name: AD_S3_BUCKET, value: {{ .Values.storage.bucket | quote }}}
- {name: AD_S3_REGION, value: {{ .Values.storage.region | quote }}}
- {name: AD_S3_FORCE_PATH_STYLE, value: {{ .Values.storage.forcePathStyle | quote }}}
{{- if .Values.storage.endpointUrl }}
- {name: AD_S3_ENDPOINT_URL, value: {{ .Values.storage.endpointUrl | quote }}}
{{- end }}
{{- if .Values.storage.serverSideEncryption }}
- {name: AD_S3_SERVER_SIDE_ENCRYPTION, value: {{ .Values.storage.serverSideEncryption | quote }}}
{{- end }}
{{- if .Values.storage.existingSecret }}
- name: AD_S3_ACCESS_KEY_ID
  valueFrom: {secretKeyRef: {name: {{ .Values.storage.existingSecret }}, key: access-key-id}}
- name: AD_S3_SECRET_ACCESS_KEY
