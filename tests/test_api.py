...
async def test_estimate_returns_faces(client: httpx.AsyncClient, two_faces_bytes: bytes) -> None:
    r = await client.post("/v1/estimate", content=two_faces_bytes, headers=IMG)
    assert r.status_code == 200
    body = r.json()
    assert body["image"] == {"width": 1024, "height": 512}
    assert len(body["faces"]) == 2
    face = body["faces"][0]
    assert set(face) == {"box", "score", "landmarks", "age"} and set(face["landmarks"]) == {
        "right_eye",
        "left_eye",
        "nose",
        "right_mouth",
    ...
    assert body["models"]["age_classifier"]["name"].startswith("levi-hassner") and body["models"]["age_views"] == 5
    age = face["age"]
    assert age["group"] in {"0-2", "4-6", "8-12", "15-20", "25-32", "38-43", "48-53", "60-100"}
    assert age["age_low"] <= age["age_high"] and age["views"] == 5
    assert sum(age["probabilities"].values()) == pytest.approx(1.0, abs=0.01)
    assert 0 <= age["confidence"] <= 1 and 0 < age["agreement"] <= 1 and isinstance(age["uncertain"], bool)
    assert body["request_id"] == r.headers["x-request-id"]


async def test_estimate_no_face_returns_empty_list(client: httpx.AsyncClient, noise_bytes: bytes) -> None:
    r = await client.post("/v1/estimate", content=noise_bytes, headers={"content-type": "image/png"})
...
async def test_annotated_endpoint_returns_jpeg_with_drawing(client: httpx.AsyncClient, astronaut_bytes: bytes) -> None:
    r = await client.post("/v1/estimate/annotated", content=astronaut_bytes, headers=IMG)
...
async def test_octet_stream_is_accepted(client: httpx.AsyncClient, astronaut_bytes: bytes) -> None:
    r = await client.post("/v1/estimate", content=astronaut_bytes, headers={"content-type": "application/octet-stream"})
    assert r.status_code == 200


async def test_wrong_content_type_is_415(client: httpx.AsyncClient, astronaut_bytes: bytes) -> None:
    r = await client.post("/v1/estimate", content=astronaut_bytes, headers={"content-type": "text/plain"})
    assert r.status_code == 415 and r.headers["content-type"].startswith("application/problem+json")
    assert r.json()["type"] == "urn:age-detection:problem:unsupported-media-type"


async def test_gif_is_415_even_with_image_content_type(client: httpx.AsyncClient) -> None:
    buf = io.BytesIO()
    Image.new("RGB", (8, 8)).save(buf, "GIF")
    r = await client.post("/v1/estimate", content=buf.getvalue(), headers=IMG)
...
async def test_invalid_image_is_422(client: httpx.AsyncClient, payload: bytes) -> None:
    r = await client.post("/v1/estimate", content=payload, headers=IMG)
...
async def test_oversize_body_is_413_by_content_length(make_client: Callable[..., Any], astronaut_bytes: bytes) -> None:
    async with make_client(max_upload_bytes=2048) as c:
        r = await c.post("/v1/estimate", content=astronaut_bytes, headers=IMG)
...
async def test_oversize_body_is_413_when_streamed_without_length(make_client: Callable[..., Any]) -> None:
    ...
    async with make_client(max_upload_bytes=2048) as c:
        r = await c.post("/v1/estimate", content=gen(), headers=IMG)  # chunked: no Content-Length
    assert r.status_code == 413
...
async def test_pixel_bomb_is_rejected(make_client: Callable[..., Any]) -> None:
    ...
    async with make_client() as c:
        r = await c.post("/v1/estimate", content=buf.getvalue(), headers={"content-type": "image/png"})
...
async def test_unknown_route_and_method_are_problem_json(client: httpx.AsyncClient) -> None:
    ...
    r = await client.get("/v1/estimate")
...
async def test_metrics_endpoint_exposes_expected_series(client: httpx.AsyncClient, astronaut_bytes: bytes) -> None:
    await client.post("/v1/estimate", content=astronaut_bytes, headers=IMG)
    text = (await client.get("/metrics")).text
    assert 'ad_http_requests_total{method="POST",route="/v1/estimate",status="200"}' in text
    assert "ad_inference_duration_seconds_bucket" in text and "ad_faces_per_image_count" in text
    assert "ad_build_info" in text
...
class TestAuth:
    ...
    async def test_wrong_key_is_401(self, secured: Any, astronaut_bytes: bytes) -> None:
        async with secured as c:
            r = await c.post("/v1/estimate", content=astronaut_bytes, headers={**IMG, "authorization": "Bearer nope"})
    ...
    async def test_valid_keys_are_accepted(self, secured: Any, astronaut_bytes: bytes) -> None:
        async with secured as c:
            for key in (self.KEY, "other"):
                r = await c.post(
                    "/v1/estimate", content=astronaut_bytes, headers={**IMG, "authorization": f"Bearer {key}"}
                )
                assert r.status_code == 200
    ...
    async def test_auth_checked_before_body_is_processed(self, secured: Any) -> None:
        async with secured as c:
            r = await c.post("/v1/estimate", content=b"garbage", headers=IMG)
...
async def test_load_shedding_returns_503_with_retry_after(
    ...
    async with make_client(max_concurrent_inference=1, inference_queue_timeout_s=0.05) as c:
        ...
        first = asyncio.create_task(c.post("/v1/estimate", content=astronaut_bytes, headers=IMG))
        await asyncio.wait_for(entered.wait(), 5)
        shed = await c.post("/v1/estimate", content=astronaut_bytes, headers=IMG)
        gate.set()
        assert (await first).status_code == 200
    assert shed.status_code == 503 and shed.headers["retry-after"] == "1"
    assert shed.json()["type"] == "urn:age-detection:problem:overloaded"
...
async def test_unhandled_errors_do_not_leak_details(
    ...
    async with make_client() as c:
        ...
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as safe:
            r = await safe.post("/v1/estimate", content=astronaut_bytes, headers=IMG)
