"""Every AD_* variable the Helm chart emits must be a real setting (a typo would be silently ignored)."""
...
CHART = Path(__file__).resolve().parent.parent / "deploy" / "helm" / "age-detection"
...
def test_chart_only_emits_known_settings() -> None:
    known = {f"AD_{name.upper()}" for name in Settings.model_fields}
    emitted: set[str] = set()
    for doc in render():
        if doc["kind"] == "ConfigMap" and "AD_LOG_LEVEL" in doc.get("data", {}):  # type: ignore[attr-defined]
            emitted |= set(doc["data"])  # type: ignore[call-overload]
        if doc["kind"] == "Deployment":
            for c in doc["spec"]["template"]["spec"]["containers"]:  # type: ignore[index]
                emitted |= {e["name"] for e in c.get("env", []) if e["name"].startswith("AD_")}
    assert emitted, "chart emitted no AD_* variables"
...
def test_chart_defaults_are_valid_settings() -> None:
    ...
    settings = Settings(
        ...
        **{k.removeprefix("AD_").lower(): v for k, v in env.items() if v != ""},
        environment="test",
    )
    assert settings.age_views == values["ageViews"] and settings.max_faces == values["maxFaces"]
