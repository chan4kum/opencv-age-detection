...
Enabled by exporting:
    AD_TEST_NATS_URL=nats://127.0.0.1:4222
    AD_TEST_S3_ENDPOINT=http://127.0.0.1:8333
    AD_TEST_S3_ACCESS_KEY=...  AD_TEST_S3_SECRET_KEY=...
...
from age_detection.api.app import create_app, load_models
from age_detection.config import Settings, hash_api_key
from age_detection.jobs import JobBus, JobMessage
from age_detection.service import InferenceService
from age_detection.storage import ObjectStore
from age_detection.worker import Worker
from tests.conftest import make_settings

NATS_URL = os.environ.get("AD_TEST_NATS_URL")
S3_ENDPOINT = os.environ.get("AD_TEST_S3_ENDPOINT")

pytestmark = [
    pytest.mark.integration,
    pytest.mark.skipif(not (NATS_URL and S3_ENDPOINT), reason="AD_TEST_NATS_URL / AD_TEST_S3_ENDPOINT not set"),
]
...
async def rig() -> AsyncIterator[Rig]:
    ...
    settings = make_settings(
        ...
        s3_access_key_id=os.environ.get("AD_TEST_S3_ACCESS_KEY", "devaccesskey"),
        s3_secret_access_key=os.environ.get("AD_TEST_S3_SECRET_KEY", "devsecretkey-change-me"),
...
async def test_worker_app_serves_probes_and_metrics(rig: Rig) -> None:
    ...
    from age_detection.worker import create_app as create_worker_app
    ...
    async with (
        ...
        assert "ad_worker_jobs_in_flight" in (await c.get("/metrics")).text
