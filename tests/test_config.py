...
def test_prod_requires_api_keys_or_explicit_opt_out() -> None:
    with pytest.raises(ValidationError, match="requires AD_API_KEY_HASHES"):
...
def test_env_parsing_of_lists(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("AD_API_KEY_HASHES", f" {H.upper()} , {hash_api_key('b')} ")
    monkeypatch.setenv("AD_CORS_ALLOW_ORIGINS", "https://a.example, https://b.example")
    monkeypatch.setenv("AD_SCORE_THRESHOLD", "0.5")
    s = Settings()
