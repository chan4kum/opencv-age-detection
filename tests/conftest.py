...
from age_detection.api.app import create_app
from age_detection.config import DEFAULT_MODEL_SHA256, Settings
from age_detection.detector import YuNetDetector

ROOT = Path(__file__).resolve().parent.parent
MODEL = ROOT / "models" / "face_detection_yunet_2026may.onnx"
DATA = Path(__file__).resolve().parent / "data"
AGE_MODEL = ROOT / "models" / "age_googlenet.onnx"


def _ensure_age_model() -> None:
    """The age weights are deliberately not committed (see docs/MODEL_CARD.md): fetch + verify them once."""
    if AGE_MODEL.is_file():
        return
    import sys

    sys.path.insert(0, str(ROOT))
    from scripts.fetch_models import fetch
