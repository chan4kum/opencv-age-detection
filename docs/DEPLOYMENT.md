...
| Grafana (dashboard "Age Detection Service") | http://localhost:3000 |
...
Auth is off by default locally; set `AD_API_KEY_HASHES` to enable it.
...
uv run age-detection keygen

helm dependency build deploy/helm/age-detection
helm install fd deploy/helm/age-detection \
  --namespace age-detection --create-namespace \
  --set auth.apiKeyHashes=<digest> \
  --set storage.endpointUrl=<s3-endpoint> --set storage.existingSecret=<secret> \
  --set api.ingress.enabled=true --set api.ingress.hosts[0].host=ages.example.com
helm test fd -n age-detection
```

Published chart and image (after a release tag): `oci://ghcr.io/chan4kum/charts/age-detection`,
`ghcr.io/chan4kum/opencv-age-detection`. Verify the signature before deploying:

```bash
cosign verify ghcr.io/chan4kum/opencv-age-detection@sha256:<digest> \
  --certificate-identity-regexp 'https://github.com/chan4kum/opencv-age-detection/.*' \
...
Minimum S3 IAM policy for the workload role (bucket `age-detection`):

```json
{
  "Version": "2012-10-17",
  "Statement": [
    {"Effect": "Allow", "Action": ["s3:PutObject", "s3:GetObject", "s3:DeleteObject"], "Resource": "arn:aws:s3:::age-detection/inputs/*"},
    {"Effect": "Allow", "Action": ["s3:ListBucket"], "Resource": "arn:aws:s3:::age-detection"}
  ]
}
```
...
* JetStream stream/KV settings are reconciled on start (`AD_NATS_PROVISION=true`). Set it to `false` and manage them yourself if you prefer.
