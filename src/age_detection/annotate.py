...
if TYPE_CHECKING:
    ...
    from age_detection.age import AgeEstimate
    from age_detection.detector import Face

_LANDMARK_COLORS = ((255, 0, 0), (0, 0, 255), (0, 255, 0), (255, 0, 255), (0, 255, 255))


def draw_faces(image_bgr: NDArray[np.uint8], faces: Sequence[tuple[Face, AgeEstimate]]) -> NDArray[np.uint8]:
    ...
    for face, est in faces:
        colour = (0, 165, 255) if est.uncertain else (0, 255, 0)
        ...
        cv2.putText(
            out,
            f"{est.group}{' ?' if est.uncertain else ''}",
            (x1, max(12, y1 - 6)),
