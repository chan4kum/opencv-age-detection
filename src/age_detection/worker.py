...
Run with ``uvicorn age_detection.worker:create_app --factory``. The process is a small FastAPI
...
from age_detection.api.app import install_error_handlers, load_models
...
from age_detection.jobs import JobBus, JobMessage, JobRecord, JobStatus
from age_detection.logging_config import configure_logging, get_logger
from age_detection.metrics import BUILD_INFO, JOB_E2E_DURATION, JOBS_PROCESSED, WORKER_IN_FLIGHT
from age_detection.schemas import HealthResponse
from age_detection.service import InferenceService
from age_detection.storage import ObjectNotFoundError, ObjectStore
from age_detection.telemetry import get_tracer, setup_tracing
...
class Worker:
    ...
    async def _process(self, msg: Msg) -> None:
        ...
        try:
            ...
            result, _, _ = await self._inference.estimate_bytes(data, source="worker")
        except Exception as exc:
