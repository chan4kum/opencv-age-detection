...
from age_detection import __version__
from age_detection.age import AgeClassifier
from age_detection.api.middleware import ObservabilityMiddleware
from age_detection.api.routes import ops, v1
from age_detection.api.state import AppState
from age_detection.config import Settings, get_settings
from age_detection.detector import YuNetDetector
from age_detection.errors import AppError
from age_detection.jobs import JobBus
from age_detection.logging_config import configure_logging, get_logger
from age_detection.metrics import BUILD_INFO
from age_detection.service import InferenceService
from age_detection.storage import ObjectStore
from age_detection.telemetry import setup_tracing
...
def load_models(settings: Settings) -> tuple[YuNetDetector, AgeClassifier]:
    ...
    classifier = AgeClassifier(
        settings.age_model_path,
        expected_sha256=settings.age_model_sha256,
        min_confidence=settings.age_min_confidence,
        crop_margin=settings.age_crop_margin,
        views=settings.age_views,
        intra_op_threads=settings.ort_intra_op_threads,
        inter_op_threads=settings.ort_inter_op_threads,
    )
    detector.warmup()
...
def problem(
    ...
    *,
    ...
    body: dict[str, object] = {
        "type": f"urn:age-detection:problem:{code}",
        "title": title,
        "status": status,
        "detail": detail,
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    app = FastAPI(
        ...
        version=__version__,
        description="YuNet age detection on ONNX Runtime. Send raw image bytes; receive boxes, scores and landmarks.",
        lifespan=lifespan,
