...
from age_detection.annotate import draw_faces, encode_jpeg
from age_detection.api.security import CurrentPrincipal
from age_detection.api.state import AppState, get_state
from age_detection.errors import (
    AsyncDisabledError,
    NotFoundError,
    PayloadTooLargeError,
    UnsupportedMediaTypeError,
)
from age_detection.imaging import probe_image
from age_detection.jobs import JobMessage, JobStatus, is_valid_job_id, new_job_id
from age_detection.metrics import IMAGES_REJECTED, JOBS_SUBMITTED
from age_detection.schemas import (
    EstimateResponse,
    HealthResponse,
    JobAccepted,
    JobResponse,
...
@v1.post(
    ...
    summary="Detect faces and estimate their age groups (synchronous)",
    description="Send the raw image bytes as the request body with an `image/*` Content-Type.",
)
async def estimate(request: Request, state: State, _: CurrentPrincipal) -> EstimateResponse:
    data = await read_image_body(request, state.settings.max_upload_bytes)
    result, _image, _faces = await state.inference.estimate_bytes(data, source="api")
    return EstimateResponse(request_id=request.state.request_id, **result.model_dump())
...
@v1.post(
    ...
    summary="Estimate age groups and return the annotated image",
)
async def detect_annotated(request: Request, state: State, _: CurrentPrincipal) -> Response:
    data = await read_image_body(request, state.settings.max_upload_bytes)
    result, image, faces = await state.inference.estimate_bytes(data, source="api")
