"""Command-line interface: local age-group estimation, webcam demo, API-key generation, model verification."""
...
from age_detection.age import ModelIntegrityError as AgeModelIntegrityError
from age_detection.age import sha256_file as age_sha256_file
from age_detection.annotate import draw_faces
from age_detection.config import DEFAULT_AGE_MODEL_SHA256, DEFAULT_MODEL_SHA256, Settings, hash_api_key
from age_detection.detector import ModelIntegrityError, YuNetDetector, sha256_file
from age_detection.errors import AppError
from age_detection.imaging import validate_and_decode
from age_detection.schemas import FaceOut
from age_detection.storage import ObjectStore
...
def _models(views: int | None, score: float | None) -> tuple[YuNetDetector, AgeClassifier]:
    ...
    classifier = AgeClassifier(
        s.age_model_path,
        expected_sha256=s.age_model_sha256,
        min_confidence=s.age_min_confidence,
        crop_margin=s.age_crop_margin,
        views=views if views is not None else s.age_views,
    )
...
def _cmd_estimate(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        return 2
    detector, classifier = _models(args.views, args.score)
    ...
    payload = {
        ...
        "faces": [FaceOut.from_face(f, e).model_dump() for f, e in results],
    }
...
def _cmd_webcam(args: argparse.Namespace) -> int:
    ...
    try:
        while True:
            ...
            cv2.imshow("age-detection (q to quit)", draw_faces(frame, results))
...
def _cmd_keygen(_: argparse.Namespace) -> int:
    ...
    print(f"AD_API_KEY_HASHES entry (put on the server):  {hash_api_key(key)}")
    return 0
...
def _cmd_verify_models(_: argparse.Namespace) -> int:
    ...
    for label, path, expected, digest in (
        ...
        ("age classifier", s.age_model_path, DEFAULT_AGE_MODEL_SHA256, age_sha256_file),
    ):
        try:
            actual = digest(path)
        except OSError as exc:
            print(f"{label}: {exc}" + ("  (run `make models`)" if label == "age classifier" else ""), file=sys.stderr)
            status = max(status, 2)
            continue
        ok = actual == expected
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="age-detection", description=__doc__)
    sub = parser.add_subparsers(dest="command", required=True)

    est = sub.add_parser("estimate", help="detect faces and estimate age groups in an image; prints JSON")
    est.add_argument("image", type=Path)
    est.add_argument("-o", "--output", type=Path, help="write an annotated copy here")
    est.add_argument("--views", type=int, help="test-time augmentation views, 1-5 (default from config)")
    ...
    cam.add_argument("--views", type=int, default=1, help="views per face (1 keeps the demo real-time)")
    ...
    except (ModelIntegrityError, AgeModelIntegrityError) as exc:
